...
export default function ManualDraftPick() {
  ...
  // --- Render ---
  return (
    <div className="flex flex-col h-screen max-h-screen bg-[#0E1117] text-gray-200 overflow-hidden font-sans selection:bg-blue-500/30">

      {/* Header / Status Bar */}
      <header className="h-16 bg-[#161B22] border-b border-gray-800 flex items-center justify-between px-6 shrink-0 shadow-lg z-20">
        <div className="flex items-center gap-4">
          <a href="/" className="w-8 h-8 flex items-center justify-center rounded-full bg-[#21262D] text-gray-400 hover:bg-gray-700 hover:text-white transition-colors mr-2">
            ←
          </a>
          <div className="flex flex-col">
            <h1 className="text-xl font-black tracking-tight text-white uppercase italic">
              <span className="text-blue-400">Tournament</span> Draft
            </h1>
            <div className="flex gap-1 h-1 mt-1 w-full bg-[#21262D] rounded-full overflow-hidden">
              {[...Array(10)].map((_, i) => (
                <div key={i} className={`flex-1 ${i < 5
                  ? (draftPicks[i] ? 'bg-blue-500' : 'bg-gray-700')
                  : (enemyDraftPicks[i - 5] ? 'bg-red-500' : 'bg-gray-700')
                  }`} />
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-col items-center">
          <div className="text-xs font-bold tracking-widest text-gray-500 uppercase mb-0.5">Current Phase</div>
          <div className={`text-xl font-black tracking-wider px-4 py-0.5 rounded flex items-center gap-2 uppercase ${activeSlot.side === 'our'
            ? (activeSlot.type === 'ban' ? 'text-blue-400 bg-blue-950/30 border border-blue-500/30' : 'text-blue-400 bg-blue-950/30 border border-blue-500/30 shadow-[0_0_20px_rgba(6,182,212,0.2)]')
            : (activeSlot.type === 'ban' ? 'text-red-400 bg-red-950/30 border border-red-500/30' : 'text-red-400 bg-red-950/30 border border-red-500/30 shadow-[0_0_20px_rgba(244,63,94,0.2)]')
            }`}>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-3 py-1.5 text-xs font-bold text-gray-400 hover:text-white hover:bg-[#21262D] rounded transition-colors uppercase tracking-wider"
          >
            Reset Draft
          </button>
        </div>
      ...
      <main className="flex-1 flex overflow-hidden relative">
        ...
        {/* LEFT: Blue Team (Our) */}
        <aside className="w-[340px] flex flex-col bg-gradient-to-r from-gray-900 to-gray-900/50 border-r border-gray-800 shrink-0 relative z-10">
          {/* Bans */}
          <div className="p-4 border-b border-gray-800/50 bg-[#161B22]/80">
            ...
            <div className="flex gap-2">
              {ourBans.map((ban, idx) => (
                <div
                  ...
                  className={`w-10 h-10 rounded-full border-2 flex items-center justify-center cursor-pointer relative overflow-hidden transition-all group ${activeSlot.side === 'our' && activeSlot.type === 'ban' && activeSlot.index === idx
                    ? 'border-blue-400 shadow-[0_0_10px_rgba(34,211,238,0.4)] scale-110 z-10'
                    : ban ? 'border-gray-600 opacity-80' : 'border-gray-800 bg-[#21262D]/50 hover:border-gray-600'
                    }`}
                >
                  {ban ? (
                    <>
                      <div className="absolute inset-0 bg-[#21262D]">
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-gray-700 text-[10px] font-bold text-gray-400">
                          {ban.substring(0, 2)}
                        </div>
                      </div>
          ...
          {/* Picks */}
          <div className="flex-1 p-4 flex flex-col gap-3 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700">
            {draftPicks.map((heroName, idx) => {
              const isActive = activeSlot.side === 'our' && activeSlot.type === 'pick' && activeSlot.index === idx;
              const lane = lanes[idx] || getDefaultLanes()[idx];
              ...
              return (
                <div
                  ...
                  className={`relative flex items-center h-24 rounded-r-xl border-l-4 transition-all cursor-pointer group ${isActive
                    ? 'bg-gradient-to-r from-blue-950/60 to-transparent border-blue-400 pl-4'
                    : heroName
                      ? 'bg-[#21262D]/40 border-blue-600/50 hover:bg-[#21262D]/60 pl-3'
                      : 'bg-[#161B22]/20 border-gray-700 hover:border-gray-600 pl-2'
                    }`}
                >
                  {/* Lane Icon Background */}
                  <div className="absolute right-2 top-1/2 -trangray-y-1/2 text-6xl text-white/5 pointer-events-none">
                    {lane.icon}
                  </div>

                  {/* Content */}
                  <div className="relative z-10 flex items-center gap-4 w-full">
                    <div className={`w-14 h-14 rounded-lg flex items-center justify-center shadow-lg border border-gray-700/50 ${heroName ? 'bg-gray-700' : 'bg-[#21262D]/50 border-dashed'
                      }`}>
                      {heroName ? (
                        <span className="text-lg font-bold text-blue-400">{heroName.charAt(0)}</span>
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      {heroName ? (
                        ...
                        <>
                          <div className="text-sm font-bold text-gray-500 uppercase tracking-wider">{isActive ? 'Picking...' : 'Empty Slot'}</div>
                          <div className="text-xs text-gray-600 mt-0.5">{lane.lane}</div>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  {heroName && (
                    <button
                      ...
                      className="absolute top-2 right-2 w-5 h-5 rounded flex items-center justify-center text-gray-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                      ✕
                    </button>
                  )}
        ...
        <section className="flex-1 flex flex-col bg-[#0E1117]/80 relative z-0">

          {/* Filters Bar */}
          <div className="p-4 border-b border-gray-800 bg-[#161B22]/90 backdrop-blur sticky top-0 z-20">
            <div className="flex gap-3 mb-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-[#21262D] border border-gray-700 text-gray-200 text-sm rounded-lg pl-10 pr-4 py-2.5 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 outline-none transition-all"
                />
              </div>
              ...
              <div className="flex gap-1">
                {uniqueRoles.map(role => (
                  <button
                    ...
                    title={role}
                    className={`w-10 h-10 rounded-lg flex items-center justify-center text-lg border transition-all ${roleFilter === role
                      ? 'bg-blue-900/50 border-blue-500 text-blue-400 shadow-lg shadow-blue-500/20'
                      : 'bg-[#21262D] border-gray-700 text-gray-500 hover:bg-gray-700 hover:text-gray-300'
                      }`}
                  >
                    {getRoleIcon(role)}
                  </button>
            ...
            <div className="flex items-center gap-4 text-xs">
              <div className="flex bg-[#21262D] rounded-lg p-1 gap-1">
                <button
                  onClick={() => setHeroListMode('all')}
                  className={`px-3 py-1.5 rounded-md font-semibold transition-all ${heroListMode === 'all' ? 'bg-gray-700 text-white shadow' : 'text-gray-500 hover:text-gray-300'}`}
                >
                  All Heroes
                </button>
                <button
                  onClick={() => setHeroListMode('recommended')}
                  className={`px-3 py-1.5 rounded-md font-semibold transition-all flex items-center gap-1 ${heroListMode === 'recommended' ? 'bg-emerald-900/30 text-emerald-400 shadow' : 'text-gray-500 hover:text-emerald-400'}`}
                >
                  <span>💡</span> Recommended
                </button>
                <button
                  onClick={() => setHeroListMode('counter')}
                  className={`px-3 py-1.5 rounded-md font-semibold transition-all flex items-center gap-1 ${heroListMode === 'counter' ? 'bg-red-900/30 text-red-400 shadow' : 'text-gray-500 hover:text-red-400'}`}
                >
              ...
              <select
                ...
                className="bg-transparent text-gray-400 font-medium focus:outline-none hover:text-white cursor-pointer"
              >
                ...
                return (
                  <div
                    ...
                    className={`
                             ...
                             ${isDisabled
                        ? 'opacity-40 grayscale border-gray-800 cursor-not-allowed'
                        : 'cursor-pointer hover:scale-105 border-gray-700 hover:border-blue-400 hover:shadow-lg hover:shadow-blue-500/20 hover:z-10 bg-[#21262D]'
                      }
                             ${isRecommended ? 'ring-2 ring-emerald-500/50' : ''}
                          `}
                  >
                    {/* Placeholder for Hero Image */}
                    <div className="absolute inset-0 bg-gradient-to-b from-gray-700 to-gray-900 flex flex-col items-center justify-center p-2 text-center">
                      <div className="text-3xl mb-1">{getRoleIcon(hero.role)}</div>
                      <div className="text-xs font-bold text-gray-200 leading-tight">{name}</div>
          ...
          {/* Analysis Footer */}
          <div className="h-auto bg-[#161B22] border-t border-gray-800 p-3 shrink-0">
            <div className="flex justify-between items-center text-xs text-gray-400 mb-2">
              <span className="font-bold uppercase tracking-wider text-gray-500">Team Analysis</span>
              <span>{composition ? `${composition.total}/5 Heroes` : 'Drafting...'}</span>
            </div>

            <div className="flex gap-4 h-16">
              {/* Stats Charts */}
              <div className="flex-1 bg-[#21262D]/50 rounded p-2 flex items-center gap-4">
                {composition ? (
                  <>
                  ...
                  <div className="text-gray-600 text-center w-full italic">Pick heroes to see analysis</div>
                )}
              </div>
              ...
              <div className="flex-1 overflow-y-auto">
                {validation.errors.length > 0 && (
                  <div className="text-red-400 text-[10px] mb-1 font-semibold">
                    ❌ {validation.errors[0]}
                  </div>
                )}
        ...
        <aside className="w-[340px] flex flex-col bg-gradient-to-l from-gray-900 to-gray-900/50 border-l border-gray-800 shrink-0 relative z-10">
          {/* Bans */}
          <div className="p-4 border-b border-gray-800/50 bg-[#161B22]/80">
            <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 flex justify-between">
              <span>Red Bans</span>
            ...
            <div className="flex gap-2 justify-end">
              {enemyBans.map((ban, idx) => (
                <div
                  ...
                  className={`w-10 h-10 rounded-full border-2 flex items-center justify-center cursor-pointer relative overflow-hidden transition-all group ${activeSlot.side === 'enemy' && activeSlot.type === 'ban' && activeSlot.index === idx
                    ? 'border-red-400 shadow-[0_0_10px_rgba(244,63,94,0.4)] scale-110 z-10'
                    : ban ? 'border-red-900/50 opacity-80' : 'border-gray-800 bg-[#21262D]/50 hover:border-gray-600'
                    }`}
                >
                  {ban ? (
                    <>
            ...
            {enemyDraftPicks.map((heroName, idx) => {
              ...
              return (
                <div
                  ...
                  className={`relative flex flex-row-reverse items-center h-24 rounded-l-xl border-r-4 transition-all cursor-pointer group ${isActive
                    ? 'bg-gradient-to-l from-red-950/60 to-transparent border-red-400 pr-4'
                    : heroName
                      ? 'bg-[#21262D]/40 border-red-600/50 hover:bg-[#21262D]/60 pr-3'
                      : 'bg-[#161B22]/20 border-gray-700 hover:border-gray-600 pr-2'
                    }`}
                >
                  {/* Lane Icon Background */}
                  <div className="absolute left-2 top-1/2 -trangray-y-1/2 text-6xl text-white/5 pointer-events-none">
                    {lane.icon}
                  </div>
                  ...
                  <div className="relative z-10 flex flex-row-reverse items-center gap-4 w-full text-right">
                    <div className={`w-14 h-14 rounded-lg flex items-center justify-center shadow-lg border border-gray-700/50 ${heroName ? 'bg-gray-700' : 'bg-[#21262D]/50 border-dashed'
                      ...
                      {heroName ? (
                        <span className="text-lg font-bold text-red-400">{heroName.charAt(0)}</span>
                      ) : (
                        <span className="text-2xl opacity-20 text-white">+</span>
                      )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      {heroName ? (
                        <>
                          <div className="text-lg font-bold text-white leading-none truncate">{heroName}</div>
                          <div className="text-xs text-red-400/80 mt-1 flex items-center gap-1 justify-end">
                  ...
                  {heroName && (
                    <button
                      ...
                      className="absolute top-2 left-2 w-5 h-5 rounded flex items-center justify-center text-gray-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                      ✕
                    </button>
                  )}
