import fs from 'fs';
import path from 'path';
import { parse } from 'csv-parse/sync';
import { validateTeamComposition } from './teamValidation.js';

// Cache for parsed data
let heroesCache = null;
let draftRulesCache = null;

// Parse draft-rules.csv (no more hardcoded magic values!)
function parseDraftRulesCSV() {
  if (draftRulesCache) return draftRulesCache;

  const filePath = path.join(process.cwd(), 'public/csv/draft-rules.csv');
  const fileContent = fs.readFileSync(filePath, 'utf-8');
  const records = parse(fileContent, { columns: true, trim: true });

  const roleCompatibility = {};
  const heroPriority = {};
  const synergyRules = [];

  records.forEach(rule => {
    const ruleType = rule['Rule Type'];
    
    if (ruleType === 'role_compatibility') {
      const primaryRole = rule['Primary Role'];
      const compatibleRole = rule['Compatible Role'];
      
      if (!roleCompatibility[primaryRole]) {
        roleCompatibility[primaryRole] = [];
      }
      roleCompatibility[primaryRole].push(compatibleRole);
    } else if (ruleType === 'hero_priority') {
      const role = rule['Primary Role'];
      const heroName = rule['Hero Name'];
      const priority = parseInt(rule['Priority'], 10);
      
      if (heroName && !isNaN(priority)) {
        if (!heroPriority[role]) {
          heroPriority[role] = {};
        }
        heroPriority[role][heroName] = priority;
      }
    } else if (ruleType === 'synergy') {
      const selectedHero = rule['Primary Role']; // Using Primary Role as SelectedHero
      const partnerHero = rule['Compatible Role']; // Using Compatible Role as PartnerHero
      const bonus = parseInt(rule['Priority'], 10); // Using Priority as Bonus
      const partnerRole = rule['Hero Name']; // Using Hero Name as PartnerRole (optional)
      
      if (selectedHero && partnerHero && !isNaN(bonus)) {
        synergyRules.push({
          selectedHero,
          partnerHero,
          bonus,
          partnerRole,
          notes: rule['Notes'] || '',
        });
      }
    }
  });

  draftRulesCache = { roleCompatibility, heroPriority, synergyRules };
  return draftRulesCache;
}

function parseHeroesCSV() {
  if (heroesCache) return heroesCache;

  const filePath = path.join(process.cwd(), 'public/csv/heroes.csv');
  const fileContent = fs.readFileSync(filePath, 'utf-8');
  const records = parse(fileContent, {
    columns: true,
    trim: true,
    // Tolerate rows with more or fewer fields than the header so Draft Pick
    // keeps working even if some skill columns are missing/extra.
    relax_column_count: true,
  });

  heroesCache = records.map(hero => ({
    name: hero['Hero Name'],
    role: hero['Role'],
    damageType: hero['Damage Type'],
    attackReliance: hero['Attack Reliance'],
    note: hero['Note'] || '',
  }));

  return heroesCache;
}

function getPrimaryRole(roleString) {
  // Extract primary role (before /)
  return roleString.split('/')[0].trim();
}

function normalizeDamageType(damageTypeString) {
  // Normalize damage type untuk comparison
  // physical_attack_speed, physical_crit -> physical
  // magic -> magic
  // physical/magic, magic/physical -> mixed
  
  if (!damageTypeString) return 'unknown';
  
  const lower = damageTypeString.toLowerCase();
  
  // Check for mixed types (both physical and magic)
  if (lower.includes('physical') && lower.includes('magic')) {
    return 'mixed';
  }
  
  // Check for physical types
  if (lower.includes('physical')) {
    return 'physical';
  }
  
  // Check for magic types
  if (lower.includes('magic')) {
    return 'magic';
  }
  
  return 'unknown';
}

function getHeroesByRole(role, heroes = null) {
  if (!heroes) heroes = parseHeroesCSV();
  return heroes.filter(hero => {
    const primaryRole = getPrimaryRole(hero.role);
    return primaryRole === role;
  });
}

function calculateSynergyBonus(selectedHeroName, partnerHero, synergyRules) {
  // Calculate synergy bonus from CSV rules (no hardcoded values!)
  // Check both specific hero synergy and role-based synergy
  
  const partnerRole = getPrimaryRole(partnerHero.role);
  let maxBonus = 0;

  synergyRules.forEach(rule => {
    // Check specific hero-to-hero synergy (e.g., Miya + Angela)
    if (rule.selectedHero === selectedHeroName && rule.partnerHero === partnerHero.name) {
      maxBonus = Math.max(maxBonus, rule.bonus);
    }
    
    // Check generic role-based synergy (e.g., Angela + Marksman role)
    if (rule.selectedHero === selectedHeroName && rule.partnerHero === partnerRole) {
      maxBonus = Math.max(maxBonus, rule.bonus);
    }
    
    // Check reverse: if partner hero has synergy with selected hero
    if (rule.selectedHero === partnerHero.name && rule.partnerHero === selectedHeroName) {
      maxBonus = Math.max(maxBonus, rule.bonus);
    }
    
    // Check reverse role-based (e.g., Marksman + Angela)
    const selectedHero = parseHeroesCSV().find(h => h.name === selectedHeroName);
    if (selectedHero) {
      const selectedRole = getPrimaryRole(selectedHero.role);
      if (rule.selectedHero === partnerHero.name && rule.partnerHero === selectedRole) {
        maxBonus = Math.max(maxBonus, rule.bonus);
      }
    }
  });

  return maxBonus;
}

function getRecommendedPartners(selectedHero, numberOfRecommendations = 4) {
  const heroes = parseHeroesCSV();
  const selected = heroes.find(h => h.name === selectedHero);

  if (!selected) {
    throw new Error(`Hero ${selectedHero} tidak ditemukan`);
  }

  const selectedPrimaryRole = getPrimaryRole(selected.role);
  const selectedDamageType = normalizeDamageType(selected.damageType);
  
  // Load rules from CSV (not hardcoded!)
  const { roleCompatibility, heroPriority, synergyRules } = parseDraftRulesCSV();
  const compatibleRoles = roleCompatibility[selectedPrimaryRole] || [];

  const recommendations = [];

  // Iterate through compatible roles dan ambil hero terbaik dari setiap role
  for (const role of compatibleRoles) {
    if (recommendations.length >= numberOfRecommendations) break;

    const heroesInRole = getHeroesByRole(role, heroes);
    if (heroesInRole.length === 0) continue;

    // Multi-factor scoring: base priority + diversity bonus + synergy bonus (CSV-based!)
    const heroesWithScore = heroesInRole.map(hero => {
      const basePriority = heroPriority[role]?.[hero.name] || 5;
      const heroDamageType = normalizeDamageType(hero.damageType);
      
      // +2 bonus if damage type berbeda (diversity)
      const diversityBonus = heroDamageType !== selectedDamageType ? 2 : 0;
      
      // Synergy bonus from CSV rules (e.g., Miya + Angela = +3)
      const synergyBonus = calculateSynergyBonus(selectedHero, hero, synergyRules);
      
      return {
        hero,
        basePriority,
        diversityBonus,
        synergyBonus,
        totalScore: basePriority + diversityBonus + synergyBonus,
      };
    });

    // Sort by total score (priority + diversity)
    heroesWithScore.sort((a, b) => b.totalScore - a.totalScore);

    if (heroesWithScore.length > 0) {
      recommendations.push(heroesWithScore[0].hero);
    }
  }

  // Fill remaining slots with best-scored heroes (not random!)
  if (recommendations.length < numberOfRecommendations) {
    const remainingHeroes = heroes.filter(h => 
      h.name !== selectedHero &&
      !recommendations.find(r => r.name === h.name)
    );

    const remainingWithScore = remainingHeroes.map(hero => {
      const role = getPrimaryRole(hero.role);
      const basePriority = heroPriority[role]?.[hero.name] || 5;
      const heroDamageType = normalizeDamageType(hero.damageType);
      const diversityBonus = heroDamageType !== selectedDamageType ? 2 : 0;
      const synergyBonus = calculateSynergyBonus(selectedHero, hero, synergyRules);

      return {
        hero,
        totalScore: basePriority + diversityBonus + synergyBonus,
      };
    });

    // Sort by score and pick top remaining heroes
    remainingWithScore.sort((a, b) => b.totalScore - a.totalScore);

    for (let i = 0; i < remainingWithScore.length && recommendations.length < numberOfRecommendations; i++) {
      recommendations.push(remainingWithScore[i].hero);
    }
  }

  return recommendations.slice(0, numberOfRecommendations);
}

// Lane-aware partner recommendation (NEW - uses DB lanes data + DB combos)
function getRecommendedPartnersWithLanes(selectedHeroWithLanes, allHeroesWithLanes, numberOfRecommendations = 4, heroCombos = []) {
  if (!selectedHeroWithLanes) {
    throw new Error('Selected hero with lanes data is required');
  }

  const selectedPrimaryRole = getPrimaryRole(selectedHeroWithLanes.role);
  const selectedDamageType = normalizeDamageType(selectedHeroWithLanes.damageType);
  const selectedLanes = (selectedHeroWithLanes.lanes || []).map(l => l.lane_name);
  const selectedPrimaryLane = selectedHeroWithLanes.lanes?.find(l => l.priority === 1)?.lane_name;

  // Load rules from CSV
  const { roleCompatibility, heroPriority, synergyRules } = parseDraftRulesCSV();
  const compatibleRoles = roleCompatibility[selectedPrimaryRole] || [];

  // Define lane priority for team composition
  const LANE_ORDER = ['Gold Lane', 'Exp Lane', 'Mid Lane', 'Jungling', 'Roaming'];
  
  // Calculate lane coverage bonus
  function calculateLaneCoverageBonus(candidateHero) {
    const candidateLanes = (candidateHero.lanes || []).map(l => l.lane_name);
    const candidatePrimaryLane = candidateHero.lanes?.find(l => l.priority === 1)?.lane_name;

    // No lane data = neutral (0 bonus/penalty)
    if (candidateLanes.length === 0) return 0;

    let bonus = 0;

    // Primary lane diversity: +5 if different primary lane
    if (candidatePrimaryLane && selectedPrimaryLane && candidatePrimaryLane !== selectedPrimaryLane) {
      bonus += 5;
    }

    // Lane overlap analysis
    const overlappingLanes = candidateLanes.filter(lane => selectedLanes.includes(lane));
    const uniqueLanes = candidateLanes.filter(lane => !selectedLanes.includes(lane));

    // +3 if hero has unique lanes (can cover different positions)
    if (uniqueLanes.length > 0) {
      bonus += 3;
    }

    // -5 penalty if ALL lanes overlap (no lane diversity)
    if (candidateLanes.length > 0 && overlappingLanes.length === candidateLanes.length) {
      bonus -= 5;
    }

    // +2 if hero can fill multiple lanes (flexible)
    if (candidateLanes.length >= 3) {
      bonus += 2;
    }

    return bonus;
  }

  const recommendations = [];

  // Iterate through compatible roles
  for (const role of compatibleRoles) {
    if (recommendations.length >= numberOfRecommendations) break;

    const heroesInRole = allHeroesWithLanes.filter(h => {
      const primaryRole = getPrimaryRole(h.role);
      return primaryRole === role && h.name !== selectedHeroWithLanes.name;
    });

    if (heroesInRole.length === 0) continue;

    // Multi-factor scoring with LANE AWARENESS + DB COMBOS
    const heroesWithScore = heroesInRole.map(hero => {
      const basePriority = heroPriority[role]?.[hero.name] || 5;
      const heroDamageType = normalizeDamageType(hero.damageType);
      
      // +2 bonus if damage type berbeda (diversity)
      const diversityBonus = heroDamageType !== selectedDamageType ? 2 : 0;
      
      // Synergy bonus from CSV rules
      const synergyBonus = calculateSynergyBonus(selectedHeroWithLanes.name, hero, synergyRules);
      
      // NEW: Lane coverage bonus
      const laneCoverageBonus = calculateLaneCoverageBonus(hero);
      
      // NEW: DB combo bonus (from hero_combos table)
      let comboBonus = 0;
      if (heroCombos && heroCombos.length > 0) {
        const combo = heroCombos.find(c => 
          (c.hero1.toLowerCase() === selectedHeroWithLanes.name.toLowerCase() && c.hero2.toLowerCase() === hero.name.toLowerCase()) ||
          (c.hero2.toLowerCase() === selectedHeroWithLanes.name.toLowerCase() && c.hero1.toLowerCase() === hero.name.toLowerCase())
        );
        if (combo) {
          // Normalize combo score: 80-95 → +3 to +5 bonus (align with CSV synergy scale)
          comboBonus = Math.min(5, Math.max(1, Math.floor((combo.synergy_score - 70) / 5)));
        }
      }
      
      return {
        hero,
        basePriority,
        diversityBonus,
        synergyBonus,
        laneCoverageBonus,
        comboBonus,
        totalScore: basePriority + diversityBonus + synergyBonus + laneCoverageBonus + comboBonus,
      };
    });

    // Sort by total score (priority + diversity + synergy + lane coverage)
    heroesWithScore.sort((a, b) => b.totalScore - a.totalScore);

    if (heroesWithScore.length > 0) {
      recommendations.push(heroesWithScore[0].hero);
    }
  }

  // Fill remaining slots with best available heroes (lane-aware)
  if (recommendations.length < numberOfRecommendations) {
    const remainingHeroes = allHeroesWithLanes.filter(h => 
      h.name !== selectedHeroWithLanes.name &&
      !recommendations.find(r => r.name === h.name)
    );

    const remainingWithScore = remainingHeroes.map(hero => {
      const role = getPrimaryRole(hero.role);
      const basePriority = heroPriority[role]?.[hero.name] || 5;
      const heroDamageType = normalizeDamageType(hero.damageType);
      const diversityBonus = heroDamageType !== selectedDamageType ? 2 : 0;
      const synergyBonus = calculateSynergyBonus(selectedHeroWithLanes.name, hero, synergyRules);
      const laneCoverageBonus = calculateLaneCoverageBonus(hero);

      // DB combo bonus (fallback path also considers combos)
      let comboBonus = 0;
      if (heroCombos && heroCombos.length > 0) {
        const combo = heroCombos.find(c => 
          (c.hero1.toLowerCase() === selectedHeroWithLanes.name.toLowerCase() && c.hero2.toLowerCase() === hero.name.toLowerCase()) ||
          (c.hero2.toLowerCase() === selectedHeroWithLanes.name.toLowerCase() && c.hero1.toLowerCase() === hero.name.toLowerCase())
        );
        if (combo) {
          comboBonus = Math.min(5, Math.max(1, Math.floor((combo.synergy_score - 70) / 5)));
        }
      }

      return {
        hero,
        totalScore: basePriority + diversityBonus + synergyBonus + laneCoverageBonus + comboBonus,
      };
    });

    remainingWithScore.sort((a, b) => b.totalScore - a.totalScore);

    for (let i = 0; i < remainingWithScore.length && recommendations.length < numberOfRecommendations; i++) {
      recommendations.push(remainingWithScore[i].hero);
    }
  }

  return recommendations.slice(0, numberOfRecommendations);
}

function simulateDraftPick(selectedHeroName) {
  const heroes = parseHeroesCSV();
  const selectedHero = heroes.find(h => h.name === selectedHeroName);

  if (!selectedHero) {
    throw new Error(`Hero ${selectedHeroName} tidak ditemukan dalam database`);
  }

  const recommended = getRecommendedPartners(selectedHeroName, 4);
  const selectedDamageType = normalizeDamageType(selectedHero.damageType);

  return {
    selected: selectedHero,
    partners: recommended,
    draftOptions: [selectedHero, ...recommended],
    recommendations: {
      pickReason: `${selectedHero.name} adalah ${getPrimaryRole(selectedHero.role)} dengan kemampuan ${selectedHero.attackReliance}`,
      partnerRoles: recommended.map(hero => {
        const partnerDamageType = normalizeDamageType(hero.damageType);
        const hasDiversity = partnerDamageType !== selectedDamageType;
        const { synergyRules } = parseDraftRulesCSV();
        const synergyBonus = calculateSynergyBonus(selectedHeroName, hero, synergyRules);
        
        return {
          name: hero.name,
          role: getPrimaryRole(hero.role),
          reason: generatePartnerReason(selectedHero, hero),
          damageType: partnerDamageType,
          diversityBonus: hasDiversity,
          synergyBonus: synergyBonus,
        };
      }),
    },
    scoring: {
      selectedDamageType,
      algorithm: 'Multi-factor: Base Priority + Diversity Bonus + Synergy Bonus (CSV) + Combo Bonus (DB)',
      factors: {
        basePriority: 'From draft-rules.csv hero_priority (1-10)',
        diversityBonus: '+2 if damage type differs',
        synergyBonus: 'From draft-rules.csv synergy rules (+0-3)',
        comboBonus: 'From hero_combos DB table (+1-5, normalized from synergy_score)',
      },
    },
  };
}

function generatePartnerReason(selected, partner) {
  const selectedRole = getPrimaryRole(selected.role);
  const partnerRole = getPrimaryRole(partner.role);

  const { synergyRules } = parseDraftRulesCSV();
  const selectedName = selected.name;
  const partnerName = partner.name;

  let bestRule = null;
  let bestBonus = 0;

  synergyRules.forEach(rule => {
    let match = false;

    if (rule.selectedHero === selectedName && rule.partnerHero === partnerName) {
      match = true;
    } else if (rule.selectedHero === selectedName && rule.partnerHero === partnerRole) {
      match = true;
    } else if (rule.selectedHero === partnerName && rule.partnerHero === selectedName) {
      match = true;
    } else if (rule.selectedHero === partnerName && rule.partnerHero === selectedRole) {
      match = true;
    }

    if (match && rule.bonus >= bestBonus) {
      bestBonus = rule.bonus;
      bestRule = rule;
    }
  });

  if (bestRule && bestRule.notes) {
    return bestRule.notes;
  }

  const reasons = {
    'Marksman-Tank': `${partner.name} sebagai tank untuk protect ${selected.name} di backline`,
    'Marksman-Support': `${partner.name} sebagai support untuk menjaga dan heal ${selected.name}`,
    'Marksman-Mage': `${partner.name} sebagai mage untuk burst damage dan crowd control`,
    'Mage-Tank': `${partner.name} sebagai tank inisiator untuk ${selected.name} follow-up damage`,
    'Mage-Support': `${partner.name} sebagai support untuk menjaga mana dan health ${selected.name}`,
    'Tank-Marksman': `${partner.name} sebagai marksman untuk high damage while ${selected.name} tank damage`,
    'Tank-Fighter': `${partner.name} sebagai fighter untuk damage dan CC combo dengan ${selected.name}`,
    'Support-Marksman': `${partner.name} sebagai marksman carry untuk diperlindungi ${selected.name}`,
    'Fighter-Mage': `${partner.name} sebagai mage untuk complementary damage type`,
    'Fighter-Support': `${partner.name} sebagai support untuk sustain dan utility`,
  };

  const key = `${selectedRole}-${partnerRole}`;
  return reasons[key] || `${partner.name} cocok bersama ${selected.name} untuk team composition yang balanced`;
}

function validateDraftTeam(heroes) {
  // Use comprehensive validation from teamValidation.js
  return validateTeamComposition(heroes);
}

// Load heroes from CSV with database-compatible format (with lanes array)
function loadHeroesFromCSVWithLanes() {
  const csvHeroes = parseHeroesCSV();
  
  // Convert CSV format to database format and add empty lanes array
  return csvHeroes.map(hero => ({
    hero_name: hero.name,
    role: hero.role,
    damage_type: hero.damageType,
    attack_reliance: hero.attackReliance,
    note: hero.note,
    lanes: [] // CSV doesn't have lane data, so empty array for fallback
  }));
}

export {
  parseHeroesCSV,
  parseDraftRulesCSV,
  getPrimaryRole,
  normalizeDamageType,
  calculateSynergyBonus,
  getHeroesByRole,
  getRecommendedPartners,
  getRecommendedPartnersWithLanes,
  simulateDraftPick,
  validateDraftTeam,
  loadHeroesFromCSVWithLanes,
};
