...
// Role compatibility matrix - hero roles yang cocok bersama
...
// Priority weights untuk hero selection
const HERO_PRIORITY = {
  'Tank': { 'Akai': 9, 'Franco': 8, 'Khufra': 8, 'Tigreal': 9, 'Grock': 8, 'Atlas': 8 },
  'Support': { 'Angela': 9, 'Lolita': 8, 'Diggie': 7, 'Estes': 8, 'Minotaur': 8 },
  'Mage': { 'Cecilion': 8, 'Eudora': 8, 'Novaria': 8, 'Lunox': 9, 'Luo Yi': 8 },
  'Fighter': { 'Hilda': 8, 'Ruby': 7, 'X.Borg': 8, 'Arlott': 8, 'Balmond': 7 },
  'Assassin': { 'Hayabusa': 8, 'Saber': 7, 'Selena': 8, 'Benedetta': 8 },
  'Marksman': { 'Miya': 7, 'Claude': 8, 'Karrie': 8, 'Layla': 6 },
};
...
function getRecommendedPartners(selectedHero, numberOfRecommendations = 4) {
  ...
  for (const role of compatibleRoles) {
    ...
    // Sort by priority atau random jika priority tidak ada
    const heroesWithPriority = heroesInRole.map(hero => ({
      hero,
      priority: HERO_PRIORITY[role]?.[hero.name] || 5,
    }));
