...
export default function RoleDistribution({ roleDistribution = [], totalHeroes = 0 }) {
  const router = useRouter()
  
  // Helper to get role icon path (simplified shapes)
  const getRoleIcon = (role) => {
    switch(role) {
      ...
      case 'Marksman': return "M3 21v-8a2 2 0 012-2h14a2 2 0 012 2v8M12 3v10m0 0l-3-3m3 3l3-3"; // Custom simplified bow/arrow idea
  ...
  const roleColors = {
    Tank: { bg: 'bg-blue-500', text: 'text-blue-700', light: 'bg-blue-50', border: 'border-blue-100', bar: 'bg-blue-500' },
    Fighter: { bg: 'bg-red-500', text: 'text-red-700', light: 'bg-red-50', border: 'border-red-100', bar: 'bg-red-500' },
    Assassin: { bg: 'bg-purple-500', text: 'text-purple-700', light: 'bg-purple-50', border: 'border-purple-100', bar: 'bg-purple-500' },
    Mage: { bg: 'bg-indigo-500', text: 'text-indigo-700', light: 'bg-indigo-50', border: 'border-indigo-100', bar: 'bg-indigo-500' },
    Marksman: { bg: 'bg-yellow-500', text: 'text-yellow-700', light: 'bg-yellow-50', border: 'border-yellow-100', bar: 'bg-yellow-500' },
    Support: { bg: 'bg-green-500', text: 'text-green-700', light: 'bg-green-50', border: 'border-green-100', bar: 'bg-green-500' },
  }
    ...
    <div className="mb-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {roleDistribution.map((role) => {
          const colors = roleColors[role.role] || { bg: 'bg-gray-500', text: 'text-gray-700', light: 'bg-gray-50', border: 'border-gray-200', bar: 'bg-gray-500' }
          const percentage = totalHeroes > 0 
            ? Math.round((role.count / totalHeroes) * 100) 
            : 0
